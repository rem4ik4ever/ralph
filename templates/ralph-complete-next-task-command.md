...
```
/ralph-complete-next-task <prd-name>
```

Where `<prd-name>` matches `.ralph/prd/<prd-name>/prd.json`
...
**IMPORTANT**: The `.ralph/prd/` directory may not be at cwd. Search for it:

1. Start at cwd
2. Check if `.ralph/prd/<prd-name>/prd.json` exists
...
find_ralph_state() {
  ...
  local dir="$PWD"
  while [[ "$dir" != "/" ]]; do
    if [[ -f "$dir/.ralph/prd/$prd/prd.json" ]]; then
      echo "$dir/.ralph/prd/$prd"
      return 0
    fi
    dir="$(dirname "$dir")"
  done
